...
function startRecursive(tempSum) {
  ...
  if (!users.length) return new Promise((resolve, reject) => { resolve(tempSum) });

  let total = tempSum ? tempSum : 0;

  const usersCount = users.length >= 10 ? 10 : users.length;
  let processArray = [];
  for (let i = 0; i < usersCount; i++) {
    processArray.push(processUser(users[i].id));
  }
  return Promise.all(processArray).then((response) => {
    response.forEach((item) => total += item);
    users.splice(0, usersCount);
    return startRecursive(total).then((res) => {
      return res;
    });
  });
}
